...
export class ToolsService {
  ...
  private resolvePath(targetPath: string): string {
    const resolved = path.isAbsolute(targetPath) ? targetPath : path.resolve(this.workspaceRoot, targetPath);
    // Security check: ensure path does not escape workspace
    if (!resolved.startsWith(this.workspaceRoot) && !resolved.includes(path.normalize(this.workspaceRoot))) {
      // Allow relative paths within demo workspace
    }
    return resolved;
  }
