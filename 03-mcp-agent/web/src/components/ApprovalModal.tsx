    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-150">
      <div
        className={`w-full max-w-xl rounded-xl border p-6 shadow-2xl bg-slate-900 ${
          isTier3 ? 'border-red-600/80 ring-1 ring-red-500/50' : 'border-cyan-600/80 ring-1 ring-cyan-500/50'
        }`}
      >
          ...
          <div className="flex items-center gap-3">
            <div
              className={`flex h-10 w-10 items-center justify-center rounded-lg ${
                isTier3 ? 'bg-red-500/10 text-red-400 border border-red-500/30' : 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/30'
              }`}
            >
              {isTier3 ? <ShieldAlert className="h-6 w-6 text-red-400" /> : <AlertTriangle className="h-6 w-6 text-cyan-400" />}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-sm font-bold text-white uppercase tracking-wider">MCP Permission Gate</h3>
                <span
                  className={`rounded px-2 py-0.5 text-[11px] font-bold ${
                    isTier3 ? 'bg-red-950 text-red-300 border border-red-800' : 'bg-cyan-950 text-cyan-300 border border-cyan-800'
                  }`}
                >
                  {action.label}
                </span>
        ...
        <div className="my-5 space-y-3 font-mono text-xs">
          <div className="flex items-center justify-between text-slate-400 text-[11px]">
            ...
            <span className="rounded bg-slate-800 px-2 py-0.5 text-slate-200 font-bold flex items-center gap-1.5">
              ...
              {action.toolName === 'edit' && <FileEdit className="h-3 w-3 text-cyan-400" />}
              {action.toolName === 'write' && <FilePlus className="h-3 w-3 text-cyan-400" />}
              {action.toolName}
            </span>
          </div>
          ...
          {action.toolName === 'edit' && (
            <div className="space-y-2">
              <div className="text-slate-300 font-semibold text-[11px]">Target: {action.arguments.path}</div>
              <div className="rounded border border-red-900/50 bg-red-950/20 p-2 text-red-300 text-[11px]">
                <div className="text-[10px] text-red-500 font-bold mb-1">- Old Text:</div>
              ...
              <div className="rounded border border-emerald-900/50 bg-emerald-950/20 p-2 text-emerald-300 text-[11px]">
                <div className="text-[10px] text-emerald-500 font-bold mb-1">+ New Text:</div>
        ...
        {showRejectInput && (
          <div className="mb-4">
            <label className="block text-[11px] text-slate-400 mb-1">Rejection reason:</label>
            <input
              type="text"
              value={rejectReason}
              onChange={(e) => setRejectReason(e.target.value)}
              placeholder="e.g. Unintended command, cancel action"
        ...
        <div className="flex items-center justify-end gap-3 pt-2 border-t border-slate-800">
          ...
          <button
            ...
            className={`rounded-lg px-5 py-2 text-xs font-bold text-white transition flex items-center gap-1.5 shadow-lg ${
              isTier3 ? 'bg-red-600 hover:bg-red-500 shadow-red-900/30' : 'bg-cyan-600 hover:bg-cyan-500 shadow-cyan-900/30'
            }`}
          >
            <Check className="h-4 w-4" />
            <span>Approve & Dispatch to MCP</span>
          </button>
        </div>
      </div>
    </div>
